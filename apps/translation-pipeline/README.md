...
브라우저에 처음 뜨는 자막(rev1)은 Gemini 응답 시간에 좌우된다. 실측에서
같은 조건으로 두 번 잰 rev1만의 지연이 이랬다.

```text
평균 1701ms   최소 843ms   최대 3229ms
```

한 세션 안에서도 최소~최대가 4배 가까이 벌어진다. 완전히 고정된 지연이
아니라 무작위 편차가 크다는 뜻이라, 같은 요청을 하나 더 보내 먼저 오는 걸
쓰는 방식(hedged request)이 최악의 경우를 줄이는 데 효과가 있다.

```text
rev1 요청 발사
  ↓ TRANSLATION_HEDGE_AFTER_MS 안에 응답이 없으면
같은 내용으로 두 번째 요청 추가 발사
  ↓
둘 중 먼저 오는 것을 쓴다
```

**발화의 첫 호출에만 건다.** 이후 조각·확정 호출까지 이중화하면 호출량이
두 배로 늘어 무료 한도에 더 빨리 닿는다. 체감에 가장 큰 영향을 주는 자리
하나로 범위를 좁혔다.

**실패에는 관여하지 않는다.** 빨리 실패하면 그대로 올린다. 늦게 온 번역은
보여줄 의미가 없다는 이유로 재시도 로직 자체를 두지 않기로 한 결정과
같은 선상이다. 두 번째 요청은 오직 "너무 느릴 때"만 나가고 "실패했을 때"는
나가지 않는다.

**완전히 고정된 지연(예: 물리적으로 먼 리전)에는 효과가 없다.** 둘 다 같은
이유로 늦으면 두 번 보내도 똑같이 늦게 온다. 무작위로 널뛰는 지연에서만
의미가 있다.

기본값 1200ms는 위 rev1 실측을 바탕으로 잡은 시작값이지 정식으로 튜닝한
값은 아니다. `TRANSLATION_HEDGE_AFTER_MS`로 조절하고, `0`이면 끈다.

**적용 뒤 다시 잰 결과다.** LiveKit Cloud(ap-south)에 배포한 상태에서, 켜기
전후로 같은 대본을 두 번씩 반복했다.

```text
                    평균      최소      최대
이중 요청 전(n=10)   1701ms    843ms   3229ms
이중 요청 후(n=10)   1398ms   1070ms   2228ms

최악의 경우: 3229ms -> 2228ms (31% 감소)
평균:       1701ms -> 1398ms (18% 감소)
```

10건 중 6건에서 실제로 발동했다. 최소값이 843ms에서 1070ms로 살짝 나빠졌는데,
이중 요청은 원래 빠른 호출에는 관여하지 않으므로 세션 간 자연스러운 편차로
본다. 표본이 각 10건뿐이라 큰 수는 아니지만, 두 세션 모두에서 일관되게
개선된 방향이라 우연으로 보기는 어렵다.
