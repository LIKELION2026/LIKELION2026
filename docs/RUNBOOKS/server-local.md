...
서버는 시작할 때 `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `SUPABASE_URL`, `SUPABASE_SECRET_KEY` 값을 검증한다. `LIVEKIT_URL`은 `wss://`로 시작해야 한다. `SUPABASE_SECRET_KEY`는 Server와 Render에만 두고 Client나 Vercel 환경변수에 넣지 않는다.
