...
1. 사용자는 피플 목록에서 연결된 팀원을 선택한다.
2. `찾아가기`를 누른다.
3. 내 아바타가 대상의 현재 위치로 이동하고, 기존 `presence.move`로 다른 화면에도 전달된다.

## 불러오기

1. 요청자가 팀원을 선택하고 `불러오기`를 누른다.
2. 대상에게 `{요청자 이름}가 당신을 불러오기를 원합니다.` 모달이 표시된다.
3. 대상은 `거절` 또는 `이동하기`를 선택한다.
4. `이동하기`를 누르면 대상 아바타가 요청자의 수락 시점 최신 위치로 이동한다.
5. 거절, 30초 만료, 연결 해제 시 아바타 위치는 바뀌지 않는다.

## 검증 시나리오

| ID | 절차 | 기대 결과 |
| --- | --- | --- |
| SU-01 | A가 B에게 찾아가기 | A가 B의 현재 위치로 이동하고 두 화면에 반영 |
| SU-02 | A가 B를 호출하고 B가 수락 | B가 A의 최신 위치로 이동 |
| SU-03 | B가 거절 | 두 아바타 위치 유지 |
| SU-04 | 요청 후 30초 대기 | 모달 종료, 두 아바타 위치 유지 |
