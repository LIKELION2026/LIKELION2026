...
Client는 `member_presence`를 수정하지 않고, 현재 유효한 `CalendarMemberStatus`를 화면 직전에 합성한다. 따라서 일정이 끝나면 기존 출근·연결·수동 협업 상태로 자동 복귀한다.

| 캘린더 상태 | 맵 아바타 | 피플 목록 |
| --- | --- | --- |
| 휴가 | 낮은 채도와 `휴가` 이름표 | 휴가 상태 배지 |
| 부재 | 낮은 채도와 `부재` 이름표 | 부재 상태 배지 |
| 재택 | `재택` 이름표 | 재택 상태 배지 |
| 회의 | `회의 중` 이름표 | 회의 상태 배지 |
| 집중 | `집중 작업` 이름표 | 집중 상태 배지 |

맵 아바타와 피플 목록은 같은 파생 상태 모델을 사용한다. 일정 변경 broadcast를 수신하면 캘린더 상태를 다시 조회해 두 화면이 함께 갱신된다.

## 화상회의 상태 연동

Meeting Lab은 LiveKit 연결이 실제로 `connected`가 된 뒤에만 같은 guest token으로 오피스 socket 세션을 복원하고 `회의 중` 상태를 broadcast한다. 회의 나가기, 연결 실패, 페이지 이탈 시에는 회의 전의 수동 협업 상태로 복구한다. 회의실 구역에 들어간 것만으로는 상태를 바꾸지 않는다.
