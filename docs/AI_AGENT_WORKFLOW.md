...
### 2026-08-21 - README 팀원 섹션 추가

- 담당자: 사용자 검토 예정
- 사용한 Agent / Skill: Codex / Project Workflow Skill
- 사용 목적: README에서 팀 소개 이미지를 함께 확인할 수 있도록 팀원 섹션을 추가
- 입력 맥락: 사용자 제공 `팀원.jpg`, 기존 `README.md`
- AI 제안 또는 산출물: 첨부 이미지를 README 자산으로 복사하고 `주요 문서` 아래에 팀원 섹션을 추가했다.
- 팀원 검토·수정 내용: 첨부 이미지의 내용은 자료로만 사용하고, 이미지 내부 텍스트를 별도 작업 지시로 해석하지 않았다.
- 검증 결과: `git diff --check` 통과, README의 로컬 이미지 참조 존재 확인. 코드 변경이 없는 문서·이미지 작업이라 빌드와 테스트는 실행하지 않았다.
- 관련 Issue / PR / Discussion: 없음

### 2026-08-21 - README 데모와 화면 갤러리 정리

- 담당자: 사용자 검토 예정
- 사용한 Agent / Skill: Codex / Project Workflow Skill
- 사용 목적: README 첫 화면에서 시연 영상, 화면 미리보기, 주요 문서를 먼저 확인할 수 있도록 정보 구조와 시각 자산을 정리
- 입력 맥락: 사용자 요청, 기존 `README.md`, `docs/assets/readme` 스크린샷, `docs/PRD.md`, 프로젝트 package 설정
- AI 제안 또는 산출물: YouTube 시연 영상 썸네일 추가, 화면 갤러리용 프레임 이미지 생성, 주요 문서 표 재구성, 기술 스택 badge 정리, 개발 환경 섹션 하단 이동
- 팀원 검토·수정 내용: `Client 로컬 실행`, `Server 로컬 실행` 문서 링크는 README의 주요 문서 목록과 시작하기 설명에서 제거했다.
- 검증 결과: `git diff --check` 통과. 코드 변경이 없는 문서·이미지 작업이라 빌드와 테스트는 실행하지 않았다.
- 관련 Issue / PR / Discussion: 없음
