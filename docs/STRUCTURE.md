...
pnpm monorepo의 장점을 활용해 Client, Server, 공통 계약을 분리한다. 이 구조는 특정 라이브러리를 중심으로 나누는 대신, 사용자 기능과 도메인 책임을 기준으로 코드를 배치한다.

현재 저장소에는 앱 코드가 없다. 아래는 초기 세팅 때 만들 목표 구조이며, 실제 구현 범위가 생긴 폴더부터 순서대로 추가한다.
